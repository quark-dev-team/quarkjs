...
a.isOpera?"O":a.isIE?"ms":""})(c);c.getElementOffset=function(a){for(var c=a.offsetLeft,d=a.offsetTop;(a=a.offsetParent)&&a!=document.body&&a!=document;)c+=a.offsetLeft,d+=a.offsetTop;return{left:c,top:d}};c.createDOMDrawable=function(a,d){var b=a.tagName||"div",g=d.image,i=a.width||g&&g.width,j=a.height||g&&g.height,k=c.createDOM(b);if(a.id)k.id=a.id;k.style.position="absolute";k.style.left=(a.left||0)+"px";k.style.top=(a.top||0)+"px";k.style.width=i+"px";k.style.height=j+"px";if(b=="canvas")k.width=
i,k.height=j,g&&(b=k.getContext("2d"),i=d.rect||[0,0,i,j],b.drawImage(g,i[0],i[1],i[2],i[3],a.x||0,a.y||0,a.width||i[2],a.height||i[3]));else if(k.style.opacity=a.alpha!=void 0?a.alpha:1,k.style.overflow="hidden",g&&g.src)k.style.backgroundImage="url("+g.src+")",k.style.backgroundPosition=-(a.rectX||0)+"px "+-(a.rectY||0)+"px";return k};c.DEG_TO_RAD=Math.PI/180;c.RAD_TO_DEG=180/Math.PI;c.hitTestPoint=function(a,c,d,b){var a=a.getBounds(),i=a.length,j=c>=a.x&&c<=a.x+a.width&&d>=a.y&&d<=a.y+a.height;
if(j&&b){for(var b=0,j=!1,k,p,q,o,l=0;l<i;l++){var m=a[l],n=a[(l+1)%i];if(m.y==n.y&&d==m.y&&(m.x>n.x?(k=n.x,p=m.x):(k=m.x,p=n.x),c>=k&&c<=p)){j=!0;continue}m.y>n.y?(q=n.y,o=m.y):(q=m.y,o=n.y);d<q||d>o||(m=(d-m.y)*(n.x-m.x)/(n.y-m.y)+m.x,m>c?b++:m==c&&(j=!0))}if(j)return 0;else if(b%2==1)return 1;return-1}return j?1:-1};c.hitTestObject=function(a,d,b){a=a.getBounds();d=d.getBounds();return(a=a.x<=d.x+d.width&&d.x<=a.x+a.width&&a.y<=d.y+d.height&&d.y<=a.y+a.height)&&b?(a=c.polygonCollision(d,d),a!==
...
function(a){a.debug=!a.debug;a._render=a.debug?function(c){c.clear!=null&&c.clear(0,0,a.width,a.height);Quark.Stage.superClass._render.call(a,c);c=a.context.context;if(c!=null)c.save(),c.lineWidth=1,c.strokeStyle="#f00",c.globalAlpha=0.5;b(a,c);c!=null&&c.restore()}:function(c){c.clear!=null&&c.clear(0,0,a.width,a.height);Quark.Stage.superClass._render.call(a,c)}}})();
...
b.prototype.getCurrentHeight=function(){return Math.abs(this.height*this.scaleY)};b.prototype.getStage=function(){for(var a=this;a.parent;)a=a.parent;return a instanceof Quark.Stage?a:null};b.prototype.toString=function(){return Quark.UIDUtil.displayObjectToString(this)}})();
